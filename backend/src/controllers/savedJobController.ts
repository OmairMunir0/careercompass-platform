...
export const getMySavedJobs = async (req: Request, res: Response) => {
  try {
    ...
    const savedJobs = await SavedJob.find({ user: req.user.id })
      .populate({
        path: "job",
        select: "title description location salaryMin salaryMax isActive createdAt recruiter",
        populate: {
          path: "recruiter",
          select: "companyName",
        },
    ...
    // Filter out saved jobs where the job itself has been deleted (orphaned records)
