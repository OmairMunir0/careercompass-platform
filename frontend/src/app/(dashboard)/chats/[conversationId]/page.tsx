...
const ChatDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchConversation = async () => {
      try {
        ...
        const { data: convRes } = await axiosInstance.get(`/api/conversations/${conversationId}`);
        const conv: Conversation = convRes.data;
        setConversation(conv);

        const otherId =
          conv.recruiterProfileId === profile.id
            ? conv.candidateProfileId
            : conv.recruiterProfileId;

        const { data: otherRes } = await axiosInstance.get(`/api/profiles/${otherId}`);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newMessageContent.trim() || !conversationId || !profile?.id) return;
        ...
        <>
          <div className="d-flex align-items-center mb-3 border-bottom pb-2">
            ...
            <div>
              ...
              <div className="text-muted small">{otherProfile.position || "No title"}</div>
            </div>
          </div>

          <div className="flex-grow-1 overflow-auto mb-3" style={{ minHeight: "400px" }}>
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`mb-2 d-flex ${
                  msg.senderProfileId === profile?.id
                    ? "justify-content-end"
              ...
              >
                <div
                  className={`p-2 rounded ${
                    msg.senderProfileId === profile?.id ? "bg-primary text-white" : "bg-light"
                  }`}
                  style={{ maxWidth: "70%" }}
                >
                  {msg.content}
          ...
          <form onSubmit={handleSendMessage} className="d-flex">
            ...
            />
            <button className="btn btn-primary" type="submit" disabled={!newMessageContent.trim()}>
              <Send size={18} />
            </button>
          </form>
        </>
